module.exports = do
  change: ->
    ...
    slides[@block-slides.idx or 0].style <<< zIndex: 9, opacity: 1
  init: ->
    @block-slides = {idx: 0}
    @change!
    move = (dir) ~>
      ...
      oldidx = @block-slides.idx
      idx = if len > 1 => (oldidx + dir + len * 2) % len else 0
      @block-slides <<< {idx, oldidx}
    ...
    btools.qs('.slides > .slide', @block).map ->
      it.style <<< opacity: 1, zIndex: 9, transform: "translate(0,0)", transition: \none
